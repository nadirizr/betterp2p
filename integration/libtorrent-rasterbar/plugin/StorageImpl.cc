...
#include "libbp2p/control/FileHandler.h"
#include "libbp2p/common/Logger.h"
#include "libbp2p/common/utility.h"
...
int StorageImpl::read(char* buf, int slot, int offset, int size)
{
    ...
    if (m_outgoing_data_cache.find(cache_key) != m_outgoing_data_cache.end()) {
        ...
        assert(size_t(offset + size) <= cache_data.size());
    ...
    if (is_bp2p_chunk) {
        ...
        assert(size_t(offset + size) <= data.size());
...
bool StorageImpl::rename_file(int index, std::string const& new_filename)
{
    ...
    if (m_handler->getMetaData().isSingleFile()) {
        ...
        assert(index >= 0 && size_t(index) < m_handler->getMetaData().containedFiles.size());
